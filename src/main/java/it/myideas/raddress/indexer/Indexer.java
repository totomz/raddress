...
public class Indexer {
	...
	public List<AddressResult> getCandidates(String input) {
		 ...
		 * Then do a Levensthein distance in the set. (TODO) 
		 */
		List<AddressResult> matches = null;
...
//			log.debug("The temporary result will be in {}", tempKey);
			...
			List<TempResult> tempRes = candidates.get().stream()
			.map(candidate -> {			    
			    return new TempResult(
			            candidate.getElement(), 
			            candidate.getScore(), 
			            pipe.hgetAll(KEY_ADDR + candidate.getElement()));
			...
			// Convert the output in an Address
			matches = tempRes.stream().map(temp -> {			    
			    Map<String, String> values = temp.response.get();
	...
	/**
	 * Returns a list of tokens. A token is 3-letter
	 * The input string is cleaned and normalize (no strange chars, lowercase)
	 * @param str
	 * @return
	 */
